 ...
 * and closes for good once you've read it.
 */
export default function WireBanner({ events }: { events: EarningsEvent[] }) {
  ...
  return (
    <Dismissible
      id={`wire.${fresh.symbol}.${fresh.month}`}
      label="Hide this report"
    >
      <Link
        href={`/t/${fresh.symbol}`}
        className="flex flex-wrap items-center gap-x-3 gap-y-1 rounded-lg border border-terminal-amber/35 bg-terminal-amber/[0.07] py-2.5 pl-4 pr-11 hover:bg-terminal-amber/[0.12]"
      >
